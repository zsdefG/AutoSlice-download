...
针对真实公网下载速度忽高忽低/掉速问题，引擎做了三点增强：

| 改进 | 说明 |
|------|------|
| 放宽读超时 | 数据读取阶段超时从固定 5s 提升到 **15s**（连接/发送仍 5s，保证 Ctrl+C 快速响应），慢速/高延迟源不再因相邻数据块间隔稍长就被误判断流 |
| 故障源可复用 | 网络/超时/5xx/提前断流等**临时失败累计 3 次（`kTempFailLimit`）才真正拉黑**该源；源恢复成功后计数清零。避免单次网络抖动永久封禁一个可用镜像，导致多源萎缩、剩余源负载集中、进一步掉速 |
| 速度平滑 | 调度器速度改为 **EMA 滑动平均**（α≈0.25），减少瞬时抖动引发"加线程/判定带宽饱和/自动限速"反复横跳 |

> 读超时阈值可通过 `DownloadOptions.receive_timeout_ms` 调整；临时失败拉黑阈值可通过 `kTempFailLimit` 调整。
